...
class Destination
  ...
  def close_metadata(destination_id, data)
    ...
    content_csv = CSV.generate { |csv|
      ...
      content.each { |id, source|
        csv << [id] + column_names.collect{ |lang| source.name&.[](lang) } + [source.attribution, source.report_issue_url_template]
      }
    }
  ...
  def close
    ...
    @destinations_schema.each{ |destination_id, row|
      ...
      logger.info("    < #{self.class.name}: #{destination_id}: +tags_schema") if row.tags_schema.present?
      logger.info("    < #{self.class.name}: #{destination_id}: +natives_schema") if row.natives_schema.present?
